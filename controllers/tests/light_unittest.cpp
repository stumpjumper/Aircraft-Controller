...
// TEST(RotatingLight, Constructor)
// {
//   uint8_t lightVariable        = 1;
//   uint32_t flatLengthvalue     = 1000; // Time light is sitting at light level flatLightLevel, between sine-wave based pulses.
//   uint8_t  flatLightLevelvalue = 10;   // The light level where the light sits for time flatLength between pulses.
//   uint32_t pulseLengthvalue    = 3000; // The half-period of the sine-wave. The total time the light is on and the value is first increasing then decreasing.
//   uint8_t  minLightLevelvalue  = 204;  // Value at which the pulsing light begins.
//   uint8_t  maxLightLevelvalue  = 20;   // Max value of the pulsing light.

//   RotatingLight light1;
...
//   // NOTE: Becuase of the way RotatingLight::update() does its indexing
//   // the arrays passed to the constructor the when update() is called
//   // for the frist time the array indexing is
//   // 1..numIntervals-1
//   // 0..numIntervals-1
...
// TEST(RotatingLight, UpdateWithTauNULL)
// {
//   const uint8_t intervals = 3*24;
...
// TEST(RotatingLight, UpdateCalledInfrequently) {
//   const uint8_t intervals = 3*24;
