...
TEST(TimeOfDayTest, SetAllTimesAndUpdateTimeOfDay) {
  ArduinoMock * arduinoMock = arduinoMockInstance();
  ...
  const uint8_t dayStartDeltaHrs   = 0;
  const uint8_t nightStartDeltaHrs = 0;
  const uint8_t eveningLengthHrs   = 0;
  const uint8_t morningLengthHrs   = 0;
  const uint8_t predawnLengthHrs   = 0;
  const TimeOfDay::DayPart currentDayPartIn = night;
  ...
  tod.setStartTimes(dayStartDeltaHrs, nightStartDeltaHrs);
  tod.setDayPartAndLenghts(eveningLengthHrs,
                           morningLengthHrs,
                           predawnLengthHrs,
                           currentDayPartIn);
